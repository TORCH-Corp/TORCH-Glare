...
/**
 * Collapsible header search. Controlled — DataViews does not filter on the
 * query itself, because in practice the search hits the backend; the consumer
 * owns the query and hands back a narrowed `data`.
 */
export function DataViewsSearch({ value, onChange, placeholder = "Search..." }: SearchProps) {
  ...
  return (
    ...
    >
      <input
        ...
        size={1}
        className="min-w-0 flex-1 bg-transparent text-[14px] leading-none text-content-presentation-global-primary caret-border-presentation-state-focus placeholder:text-content-presentation-global-tertiary focus:outline-none"
      />
      <button
        type="button"
