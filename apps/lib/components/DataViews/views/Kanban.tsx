...
import type { DynamicRecord, KanbanColumnColor } from "../types";
...
export type KanbanProps = {
  /** Field path whose value groups records into columns. Falls back to
   *  `config.kanbanGroupBy`, then to `"status"`. */
  groupBy?: string;
...
/** Drag-and-drop board grouped by an enum field. Registers itself as the
 *  `kanban` view; renders only while it is the active one. */
export function DataViewsKanban({ label = "Board", ...props }: KanbanProps) {
  const active = useRegisterView({ id: "kanban", label, icon: <LayoutGrid /> });
  return active ? <KanbanBodyView {...props} /> : null;
}

function KanbanBodyView({ groupBy, titleField, onColumnAction, className }: KanbanProps) {
  const { fields, config, updateRecord } = useDataViews();
  const { records: data, displayFields, idPath } = useViewData();
  const groupByField = groupBy ?? config.kanbanGroupBy ?? "status";
  ...
  if (fields.length > 0 && !groupField) {
    devWarn(
      `kanban-groupBy:${groupByField}`,
      `<DataViews.Kanban groupBy="${groupByField}"> does not match any field. ` +
        `Every record will land in one column. Known paths: ${fields.map((f) => f.path).join(", ")}.`,
    );
  }
  ...
  const handleDragStart = (id: string, columnId: string) => {
  ...
  const handleDrop = (targetColumnId: string) => {
    ...
    updateRecord(draggedItem.id, (record) => setByPath(record, groupByField, targetColumnId));
    setDraggedItem(null);
    setDragOverColumnId(null);
  };
  ...
  const renderCard = (item: DynamicRecord, idx: number) => {
    ...
    const isDraggingThis = draggedItem?.id === itemId;
    ...
    return (
      <DataViewCard
        ...
        onDragStart={
          !isMobile
            ? () =>
                handleDragStart(itemId, String(getByPath(item, groupByField) ?? "Uncategorized"))
            : undefined
        }
        ...
        <div className="h-full overflow-y-auto p-4 bg-background-presentation-body-primary">
          <div className="flex flex-col gap-4">
            {kanbanColumns.map((column) => (
              <div key={column.id} data-column-id={column.id} className="flex flex-col gap-3">
