...
export const TreeDropDown = ({ variant = "secondary", treeLabel, open, theme, ...props }: Props) => {
    const [isActive, setIsActive] = useState(open);

    return (
        <div {...props} className={cn("flex h-fit flex-col transition-all ease-in-out duration-500")}>
            <div onClick={() => setIsActive(!isActive)} data-theme={theme} className={cn(TreeDropDownVariants({ variant, active: isActive }))}>
                <button className={cn("outline-none border-none leading-0 transition-transform ease-in-out flex justify-center items-center bg-background-system-body-tertiary h-[28px] w-[28px] rounded-full text-[20px] text-content-system-global-primary", { "rotate-180": isActive })}>
                    <i className={cn("leading-none ri-arrow-down-s-line ")}></i>
                </button>
                <p className={cn("text-content-system-global-primary typography-body-medium-medium transition-all ease-in-out duration-100")}>{treeLabel}</p>
            </div>
            <div className={cn("mt-0 pl-[22px] relative overflow-scroll scrollbar-hide transition-all duration-500 ease-in-out", { "max-h-[2000px] mt-1": isActive, "max-h-0": !isActive })}>
                <span className="h-full w-[1px] bg-border-system-global-primary absolute left-[21px] top-0 rounded-sm z-10" />
                <div className="h-full flex flex-col gap-1">{props.children}</div>
            </div>
        </div>
    );
};
