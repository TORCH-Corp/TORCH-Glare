...
export function Layout({ ...props }: LayoutProps) {
  return (
    <div {...props} className="flex h-screen gap-[10px] p-[10px] bg-background-system-body-base">
      {props.children}
    </div>
  );
}
...
export function Content({ ...props }: ContentProps) {
  return (
    <main {...props} className={cn("flex flex-grow flex-1 overflow-hidden", props.className)}>
      <div className="flex p-1 rounded-xl bg-background-system-body-tertiary shadow-[0px_0px_18px_0px_rgba(0,0,0,0.75)] flex-1 flex-grow">
        <div
...
export function SideBar({ children, headerChild, navigationChildren, iconButtons, ...props }: SideBarProps) {
  return (
    <aside
      {...props}
      className={cn("flex w-[265px] p-1 flex-shrink-0 items-start rounded-xl bg-background-system-body-tertiary shadow-[0px_0px_18px_0px_rgba(0,0,0,0.75)] h-full", props.className)}
    >
      <div className="grid grid-rows-[56px_1fr] w-full rounded-lg border border-border-system-global-primary h-full gap-[1px]">
        {children ? (
          children
        ) : (
          <>
