...
const inputFiles = glob.sync("tailwindPlugins/**/index.js");
  ...
  build: {
    rollupOptions: {
      input: Object.fromEntries(
        inputFiles.map((file) => [
          path.relative("tailwindPlugins", file).replace(/\/index\.js$/, ""), // Use folder name as key
          path.resolve(file), // Absolute path to the file
        ])
      ),
      output: {
        format: "es", // ESM output
        dir: "tailwindPlugins", // Keep build files in plugin/
